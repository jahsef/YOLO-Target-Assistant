...
#
# CSV columns (10):
# wma_factor, rsi_factor, age_factor, raw_lead_sens, lead_sensitivity(ema),
# wma_velocity, lead_pixels_x, lead_pixels_y, raw_delta_x, raw_delta_y

sns.set_theme(style="darkgrid")
palette = sns.color_palette("tab10")

data = np.genfromtxt('dampening_factors.csv', delimiter=',')
(wma_factor, rsi_factor, age_factor, raw_lead_sens, lead_sens_ema,
 wma_velocity, lead_px_x, lead_px_y,
...
ax.plot(frames, wma_factor, color=palette[0], label='wma_factor (velocity)', linewidth=1.5)
ax.plot(frames, rsi_factor, color=palette[4], label='rsi_factor (oscillation)', linewidth=1.5)
ax.plot(frames, age_factor, color=palette[2], label='age_factor (track warmup)', linewidth=1.5)
...
# --- Row 4: Raw deltas with EMA ---
ax = axes[3]
ax.set_title('Raw Deltas', fontweight='bold')
